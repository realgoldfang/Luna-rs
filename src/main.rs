...
fn add_python_xml_to_tns<W: Write + Seek>(
    ...
    if filename.len() > 240 {
        eprintln!("Python script filenames limited to 240 characters");
...
fn main() -> io::Result<()> {
    ...
    let mut tiversion: u32 = 0x0500;

    for i in 1..args.len() - 1 {
        if has_ext(&args[i], ".bmp") {
            tiversion = 0x0700;
            break;
        }
    }

    let outfile_path = &args[args.len() - 1];
    let _ = fs::remove_file(outfile_path);

    let output_file = fs::File::create(outfile_path)?;
    let mut writer = zip_writer::TiZipWriter::new(io::BufWriter::new(output_file));

    // Document.xml must be added first to the TNS
    let mut has_processed_documentxml = false;
    for i in 1..args.len() - 1 {
        if gnu_basename(&args[i]) == "Document.xml" {
            println!("processing '{}'...", args[i]);
            add_infile_to_tns(&mut writer, &args[i], tiversion)?;
            has_processed_documentxml = true;
            break;
        }
    }
    ...
    let mut is_converting_lua = false;
    let mut added_python_xml = false;
    ...
    for i in 1..args.len() - 1 {
        ...
        if has_ext(&args[i], ".lua") || args[i] == "-" {
            if is_converting_lua {
                println!("[WARN] skipping it, can only add a single Lua script to the TNS file");
                continue;
            }
            is_converting_lua = true;
        }

        if !added_python_xml && has_ext(&args[i], ".py") {
            add_python_xml_to_tns(&mut writer, &args[i], tiversion)?;
            added_python_xml = true;
        }
